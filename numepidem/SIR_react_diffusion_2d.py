...
def run_test (
    ...
    if verbosity: print("Building mesh", mesh_file)
    mesh = Mesh(mesh_file)
    ...
    # Initial data
    #
    madrid_coordinates = -3.7025600, 40.4165000 # Latitude, longitude
    Sx0, Sy0 = madrid_coordinates
    S_init = Expression ( "C+C/100*exp(-C/100*( pow(x[0]-Sx0, 2) + pow(x[1]-Sy0, 2)))",
                          C=325, Sx0=Sx0, Sy0=Sy0, degree = 1 )
    S0 = interpolate(S_init, Vh)

    # I_init = Expression ( "C/2-C*abs(x[0]-0.5)", C=7500, degree = 1 )
    I_init = Expression ( "C*exp(-C/10*( pow(x[0]-Sx0, 2) + pow(x[1]-Sy0, 2) ))",
                          C=75, Sx0=Sx0, Sy0=Sy0, degree = 1 )
    ...
    SvtkFile = File("testS.pvd")
    IvtkFile = File("testI.pvd")
    RvtkFile = File("testR.pvd")
    SvtkFile << S0
    IvtkFile << I0
    RvtkFile << R0
